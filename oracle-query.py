<template>
  <card>
    ...
    <div v-if="showAmortizationChart" class="mt-4">
      <h4 class="text-center mb-4">Amortization Graph</h4>
      <AmortizationChart :scheduleData="amortizationData"/>
    </div>
  </card>
...
export default {
  data() {
    return {
      ...
      // for the amortization process
      amortizationData: null,
      showAmortizationChart: false
    };
  },
  computed: {
  ...
  methods: {
    ...
    async submitAmortizationSchedule() {
      try {
        ...
        Vue.set(this, 'amortizationData', response.data);
